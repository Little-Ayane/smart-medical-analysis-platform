...
```
                    ┌── dim_hospital      (医院, 218 家)
                    ├── dim_patient       (患者人口统计, 10,145 种组合)
                    ├── dim_diagnosis     (CCSR 诊断, 485 种)
fact_discharge ─────├── dim_procedure     (CCSR 手术/操作, 322 种)
 (住院事实表)        ├── dim_drg           (APR-DRG 分组, 5,761 种)
 10,378,775 行      ├── dim_payment       (支付方式组合, 551 种)
 5年(2020-2024)     └── dim_time          (时间维度, 5 年)
```
...
|------|------|------|
| `year_id` | INT PK | 自增主键 |
| `discharge_year` | INT | 出院年份（2020-2024） |
...
---
数据已严格清洗：移除 Gender='U'（1,277行）、医院ID缺失（38,050行），共删除 38,680 条异常记录。性别字段仅含 M/F。5 年数据分布验证通过，外键完整性 7/7 全通过（0 孤儿ID）。


## 原始宽表（ODS 层）

同时保留了原始清洗后的宽表 `medical_data`（10,378,775 行，33 列），字段与原始 CSV 一一对应，用于追溯和临时查询。

---

## 数据来源

纽约州住院出院数据（SPARCS），覆盖 2020-2024 共 5 个年度。清洗后保留率 99.63%，数据质量报告见 `data-analysis/docs/data-quality-report.md`。
