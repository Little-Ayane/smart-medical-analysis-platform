...
import api from '@/api'
import { AGE_GROUP, DISPOSITION, t } from '@/utils/labels'
  ...
  try {
    ...
    if (tabKey === 'emergency-rate') {
      option = {
        ...
        yAxis: { type: 'value', name: '急诊占比 (%)', min: 0 },
        series: [{
          type: 'line',
      ...
      option = {
        ...
        xAxis: { type: 'category', data: data.map(d => t(AGE_GROUP, d.key)), axisLabel: { rotate: 20 } },
      ...
      const dispositionLabels = dispositions.map(d => t(DISPOSITION, d))
      ...
      option = {
        tooltip: { trigger: 'axis' },
        grid: { left: '6%', right: '4%', top: '8%', bottom: '18%' },
        xAxis: { type: 'category', data: dispositionLabels, axisLabel: { rotate: 20, interval: 0 } },
