...
import api from '@/api'
import { AGE_GROUP, PAYMENT, SEVERITY, t } from '@/utils/labels'
...
const loadData = async (tabKey) => {
  ...
  try {
    ...
    switch (tabKey) {
      case 'composition': {
        // 使用预聚合表数据，与大屏保持一致
        const overview = await api.getBigscreenOverview()
        const paymentData = overview.data?.payment_types || []
        result = { data: paymentData.map(d => ({ key: d.payment_type, value: d.cases, count: d.cases, pct: d.percentage })) }
        break
      }
      case 'cross':
    ...
    if (tabKey === 'composition') {
      const pieData = data.map(d => ({ name: t(PAYMENT, d.key), value: d.value || d.count || 0 }))
      option = {
      ...
      const keyLabels = keys.map(k => t(PAYMENT, k))
      const dim2s = [...new Set(data.map(d => d.dim2_name || d.dim2))]
      const dim2Labels = dim2s.map(s => t(AGE_GROUP, s))
      ...
      option = {
        ...
        xAxis: { type: 'category', data: keyLabels, axisLabel: { rotate: 15, fontSize: 10 } },
        yAxis: { type: 'value', name: '人数' },
        legend: { top: 0, data: dim2Labels, textStyle: { color: '#8ab4d6' } },
        series
      }
      ...
      const trNode = (name) => {
        ...
        return idx >= 0 ? name.slice(0, idx + 1) + t(PAYMENT, name.slice(idx + 1)) : t(PAYMENT, name)
      }
      const nodes = (sankeyData.nodes || []).map(n => ({ ...n, name: trNode(n.name) }))
      const links = (sankeyData.links || []).map(l => ({ ...l, source: trNode(l.source), target: trNode(l.target) }))
      option = {
        tooltip: { trigger: 'item' },
      ...
      const keys = data.map(d => t(AGE_GROUP, d.key))
      ...
      const d = data
      const sevData = Object.entries(d.severity_distribution || {}).map(([k, v]) => ({ name: t(SEVERITY, k), value: v }))
      option = {
