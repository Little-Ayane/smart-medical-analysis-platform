...
by ∈ payment/age_group/diagnosis；返回 {key,count,avg_charges,avg_costs,charge_cost_ratio}。
数据源：medical_db 星型模型。
"""
from flask import request

from common import envelope, execute_cached, fetch_dim_names, parse_choice, parse_top
from star_common import merge_joins, parse_filters_star
from .._shared import _endpoint, payment_bp

COST_BY = {"payment": ("f.payment_typology_1", "fact", "", None),
           "age_group": ("f.age_group", "fact", "", None),
           "diagnosis": ("f.diagnosis_id", "dim", "", "diagnosis_id")}
...
def cost_relation():
    def handler(args):
        ...
        col, kind, own_join, fk = COST_BY[by]
