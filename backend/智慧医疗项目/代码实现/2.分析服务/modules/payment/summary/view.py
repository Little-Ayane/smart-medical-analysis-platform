...
3 条 SQL 一个缓存 key。数据源：medical_db 星型模型。
"""
...
def summary():
    def handler(args):
        ...
        pay_join = " ".join(merge_joins([""], fjoins))
        drg_join = " ".join(merge_joins([""], fjoins))
        ...
        row1, _ = timed_query(
            ...
            f"SUM(f.payment_typology_1='Self-Pay') AS sp, "
            f"SUM(f.emergency_department_indicator='Y') AS ed "
            f"FROM fact_discharge f {pay_join} {where_sql}", params)
        sev_rows, _ = timed_query(
            f"SELECT f.apr_severity_code AS k, COUNT(*) AS c "
            f"FROM fact_discharge f {drg_join} {where_sql} "
            f"GROUP BY f.apr_severity_code", params)
        pay_rows, _ = timed_query(
            f"SELECT f.payment_typology_1 AS k, COUNT(*) AS c "
            f"FROM fact_discharge f {pay_join} {where_sql} "
            f"GROUP BY f.payment_typology_1 ORDER BY c DESC LIMIT 1", params)
