...
# —— 反规范化后 fact_discharge 已到 1038 万行（2020~2024），不传 year 会全表 GROUP BY 超时 ——
# 默认分析年份：未显式指定 year 时强制该年份，走 discharge_year 覆盖索引（~亚秒级过滤）。
# P3 的 _norm_year 目前也只放行 2021，故默认 2021 与 P3 口径一致。
DEFAULT_YEAR = int(os.getenv("DEFAULT_YEAR", "2021"))
# 按年份分组的端点（year 是聚合维度而非过滤条件），这些端点不强制默认年份。
_YEAR_DIMENSION_HINTS = {"cost_trend"}          # 费用趋势（按年分组）
_YEAR_DIMENSION_ENDPOINTS = {"/analysis/trend"}  # 遗留趋势（按年分组）

# —— 通用模式联网搜索（Bing 国内可访问，无 key；失败自动降级纯 LLM）——
WEB_SEARCH_ENABLED = os.getenv("WEB_SEARCH_ENABLED", "1").lower() in ("1", "true", "yes", "on")
WEB_SEARCH_TOP_N = max(1, int(os.getenv("WEB_SEARCH_TOP_N", "5")))
...
def _call_llm_safely(messages: list[dict], use_small_model: bool = False) -> tuple[bool, str]:
    ...
    # 小米 MiMo 是推理模型：先输出 reasoning_content 再输出 content，max_tokens 需足够大
    # （否则思考就把额度用光、content 为空）。默认 8000，可用 LLM_MAX_TOKENS 覆盖。
    ...
    try:
        ...
        body = {
            ...
            "stream": False,
            # 小米 MiMo 默认开启深度思考（先 reasoning_content 后 content），
            # 会拖慢响应（实测摘要 110s+）。关闭 thinking 让它直接产出答案。
...
def _call_llm_stream(messages: list[dict], on_token, use_small_model: bool = False) -> tuple[bool, str]:
    ...
    try:
        ...
        body = {
            ...
            "max_tokens": max_tokens,
            "stream": True,
            # 同 _call_llm_safely：关闭 MiMo 深度思考，避免响应被 reasoning 拖慢
            "thinking": {"type": "disabled"},
        }
        content = ""
...
def _build_cost_relation_params(intent: dict) -> dict:
    ...
    if by not in ("payment", "age_group", "diagnosis"):
        by = "payment"
    params = {"by": by, "top": min(intent.get("top", 30), 100)}
    # P3 year 白名单仅放行 2021（common.py 的 _norm_year）；不传 year 会触发
    # 全表 210 万行 GROUP BY 导致 15s×2 次重试超时。故未显式指定年份时强制
    # 默认 2021（与 P3 实际可用数据口径一致），确保走小表秒回。
    y = intent.get("year")
    params["year"] = int(y) if y else 2021
    return params
...
def _rank_top(intent: dict, default: int = 20) -> int:
    """排行类端点 top 下限保护：当请求的是「最多/最高的一项」(top<=1) 时，
    强制拉取 Top5 作为图表排行上下文（单柱排行图无意义）；其余按请求值截断到 100。
    文字答案仍取返回数据的 data[0]（最高项），不受影响。"""
    req = intent.get("top", default)
    return 5 if (req or 0) <= 1 else min(req, 100)
...
def _build_top_diagnoses_params(intent: dict) -> dict:
    ...
    params = {"metric": metric, "top": _rank_top(intent)}
    if degraded:
        params["__metric_degraded"] = True
    return params
...
ROUTE_TABLE = {
    # —— 模块一：病种与手术分析（7 个）——
    "top_diagnoses":       {"endpoint": "/disease/top-diagnoses",    "build": _build_top_diagnoses_params,       "timeout": 30},
    "top_procedures":      {"endpoint": "/disease/top-procedures",   "build": _build_top_procedures_params,      "timeout": 30},
    "severity_profile":    {"endpoint": "/disease/severity-profile", "build": _build_severity_profile_params,    "timeout": 30},
    "population_diff":     {"endpoint": "/disease/population-diff",  "build": _build_population_diff_params,     "timeout": 30},
    "pyramid":             {"endpoint": "/disease/pyramid",         "build": _build_pyramid_params,             "timeout": 30},
    "region_diff":         {"endpoint": "/disease/region-diff",      "build": _build_region_diff_params,         "timeout": 30},
    "heatmap":             {"endpoint": "/disease/heatmap",          "build": _build_heatmap_params,             "timeout": 30},
    # —— 模块二：支付分析（6 个）——
    "payment_composition": {"endpoint": "/payment/composition",      "build": _build_payment_composition_params, "timeout": 30},
    "payment_cross":       {"endpoint": "/payment/cross",            "build": _build_payment_cross_params,       "timeout": 30},
    "sankey":              {"endpoint": "/payment/sankey",           "build": _build_sankey_params,              "timeout": 20},
    "cost_relation":       {"endpoint": "/payment/cost-relation",    "build": _build_cost_relation_params,       "timeout": 30},
    "oop_burden":          {"endpoint": "/payment/oop-burden",       "build": _build_oop_burden_params,          "timeout": 30},
    "payment_summary":     {"endpoint": "/payment/summary",          "build": _build_payment_summary_params,    "timeout": 30},
    ...
    "quality_overview":    {"endpoint": "/quality/overview",         "build": _build_quality_overview_params,    "timeout": 30},
    "quality_mortality":   {"endpoint": "/quality/mortality",        "build": _build_quality_mortality_params,   "timeout": 30},
    "quality_los":         {"endpoint": "/quality/length-of-stay",   "build": _build_quality_los_params,         "timeout": 30},
    "quality_facility":    {"endpoint": "/quality/facility-ranking", "build": _build_quality_facility_params,    "timeout": 30},
    "quality_disposition": {"endpoint": "/quality/disposition",      "build": _build_quality_disposition_params, "timeout": 30},
}
...
def _dispatch_chart_hint(chart_hint: str, **kwargs) -> dict:
    ...
    # 反规范化后全表 1038 万行：未指定 year 时全表 GROUP BY 会超时。
    # 非「按年分组」的端点默认补 DEFAULT_YEAR（2021），走年份索引秒回。
    if chart_hint not in _YEAR_DIMENSION_HINTS and "year" not in filters:
...
def _dispatch_legacy(endpoint: str, timeout: int, **kwargs) -> dict:
    ...
    """
    filters = kwargs.get("filters") or {}
    # 同 _dispatch_chart_hint：非趋势端点默认补年份，避免全表 GROUP BY 超时。
    if endpoint not in _YEAR_DIMENSION_ENDPOINTS and "year" not in filters:
        filters["year"] = DEFAULT_YEAR
...
class _CostRelationInput(BaseModel):
    ...
    year: int = Field(default=2021, description="年份筛选，默认2021（当前数据仅含2021年，仅支持2021）")
    filters: dict = Field(default_factory=dict, description=_FILTERS_DESC)
...
def _execute_cost_relation(by: str = "payment", top: int = 30,
                           year: int = 2021, filters: dict = None) -> dict:
    """查询费用-成本关系散点图（费用与成本的对比关系）。
    适用于"费用关系""成本对比""费用散点图""成本费用"等问法。"""
    return _dispatch_chart_hint("cost_relation", by=by, top=top, year=year, filters=filters)
...
if _LANGCHAIN_AVAILABLE:
    _TOOLS = [
        ...
        StructuredTool.from_function(
            ...
            description=(
                "分析住院费用与成本之间的关系（散点图呈现，对比不同维度的费用水平、成本水平与费本比）。"
                "适用于'费用关系''成本对比''费用散点图''成本费用'"
                "'费用和成本之间有什么关系''费用和成本是什么关系''两者的关系如何'"
                "'费用与成本的关联''费用成本相关性'等问法。"
                "当用户询问费用与成本如何关联、谁高谁低、费本比如何时优先选用。"),
...
SUGGESTED_QUESTIONS: list[dict] = [
    ...
    {"tool": "cost_trend", "category": "费用成本",
     "question": "住院总费用的年度变化趋势？"},
    # —— 医疗质量类（对接 /quality/*）——
    {"tool": "quality_overview", "category": "医疗质量",
     "question": "医疗质量整体指标总览？"},
    {"tool": "quality_mortality", "category": "医疗质量",
     "question": "各病种的死亡率排行？"},
    {"tool": "quality_los", "category": "医疗质量",
     "question": "各病种的平均住院日排行？"},
    {"tool": "quality_facility", "category": "医疗质量",
     "question": "各医院的质量对比排名？"},
    {"tool": "quality_disposition", "category": "医疗质量",
     "question": "患者离院去向的构成？"},
    # —— 通用分析类 ——
    {"tool": "general_aggregate", "category": "通用分析",
     "question": "各年龄段的住院人数统计？"},
]
...
- "老年人的住院疾病排行"         → 调用 top_diagnoses(filters={"age_group":"70 or Older"}, top=20)
...
- "2021年住院人数最多的疾病"     → 调用 top_diagnoses(filters={"year":2021}, metric="count", top=10)
...
def _is_empty_api_result(api_result) -> bool:
    """判断 api_result 是否为空结果（无错误但 data 为空）。"""
    ...
    if isinstance(data, dict):
        return not data  # 空 dict（sankey nodes/links、KPI 等）
    return not data


def _relax_tool_args(tool_name: str, args: dict) -> dict:
    """空结果兜底：去掉模型很可能填错的值类参数再重试。

    模型常把中文病名/手术名/医院名当过滤值传给 P3，但库里存的是英文编码/名称，
    导致查不到。这里去掉 dimension_value 及诊断/手术/医院/地区/支付/严重程度等
    自由文本过滤，只保留 year/gender/age_group 这类标准化枚举 + 结构参数
    （metric/top/group/level 等）。cost_trend 额外去掉 dimension → 整体趋势。
    """
    if not isinstance(args, dict):
        return {}
    relaxed = dict(args)
    relaxed.pop("dimension_value", None)
    if tool_name == "cost_trend":
        relaxed.pop("dimension", None)  # 去掉维度 → 整体趋势（避免"按诊断分组的巨量结果"）
    f = relaxed.get("filters")
    if isinstance(f, dict):
        relaxed["filters"] = {
            k: v for k, v in f.items() if k in ("year", "gender", "age_group")
        }
    return relaxed


def _invoke_tool_by_name(tool_name: str, tool_args: dict):
    """按名称在 _TOOLS 中查找并执行工具，返回 api_result 或 None。

    StructuredTool.invoke 会走 Pydantic 参数校验；args 不完整时先按原参试一次，
    失败则退化为默认参数再试。若返回空结果，再按放宽后的参数重试一次（见
    _relax_tool_args），避免因模型生成的过滤值对不上英文数据而直接"未查询到结果"。
    """
    for t in _TOOLS:
        ...
        if not isinstance(result, dict):
            return None
        # 空结果 → 放宽重试（去掉值类过滤）
        if _is_empty_api_result(result):
            relaxed = _relax_tool_args(tool_name, tool_args or {})
            if relaxed != (tool_args or {}):
                try:
                    relaxed_result = t.invoke(relaxed)
                except Exception as e:
                    logger.warning("[Agent] 放宽重试工具 %s 失败：%s", tool_name, e)
                    relaxed_result = None
                if isinstance(relaxed_result, dict) and not _is_empty_api_result(relaxed_result):
                    relaxed_result.setdefault("meta", {})["filters_relaxed"] = True
                    logger.info("[Agent] 空结果已放宽重试命中：%s（原过滤 %s）",
                                tool_name, json.dumps(tool_args or {}, ensure_ascii=False)[:200])
...
def _get_tool_calling_llm():
    ...
    try:
        llm = ChatOpenAI(
            ...
            timeout=LLM_TIMEOUT,
            # 关闭 MiMo 深度思考（工具调用不应 reasoning，否则偶发 5~60s）
...
def _repair_truncated_json(raw: str) -> str:
    """修复被截断的 JSON（SiliconFlow Qwen 偶发丢掉结尾的 }/]）。

    只补全缺失的闭合括号，不改变已有内容；无法判断时原样返回。
    """
    if not raw:
        return raw
    stack = []
...
def _parse_llm_json_args(raw) -> dict:
    """解析工具调用的 arguments（可能为 str/dict，str 可能被截断）。"""
...
def _extract_native_tool_call(resp):
    """从原生 tool-calling 响应里提取 (tool_name, args)。

    SiliconFlow 的 Qwen 模型常把 tool_calls 放到 additional_kwargs 而非
    标准 .tool_calls 字段；这里两处都读，并对截断的 arguments 做修复。
    返回 (name, args_dict)，提取失败返回 None。
    """
    # 1) 标准 langchain tool_calls
    for tc in getattr(resp, "tool_calls", None) or []:
        raw_name = tc.get("name", "") if isinstance(tc, dict) else getattr(tc, "name", "")
        name = _normalize_tool_name(raw_name or "")
        if name:
            args = tc.get("args", {}) if isinstance(tc, dict) else getattr(tc, "args", {}) or {}
            return name, (args if isinstance(args, dict) else {})
    # 2) additional_kwargs 里的原始 OpenAI 格式
    raw_calls = (getattr(resp, "additional_kwargs", {}) or {}).get("tool_calls") or []
    for tc in raw_calls:
        if not isinstance(tc, dict):
            continue
        fn = tc.get("function") or {}
        name = _normalize_tool_name(fn.get("name") or "")
        if not name:
            continue
        return name, _parse_llm_json_args(fn.get("arguments"))
    return None
...
def _run_native_tool_calling(question, with_report, conversation_id, llm,
    ...
    for attempt in range(1 + AGENT_TOOL_RETRIES):
        ...
        # 1) 提取工具调用（兼容标准 .tool_calls 与 additional_kwargs 两种落点，
        #    并对 SiliconFlow Qwen 偶发截断的 arguments 做修复）
        picked = _extract_native_tool_call(resp)
        if picked:
            name, args = picked
...
_EN_ZH_MAP = {
    ...
    # 地区
    "OOS": "州外",
    # 高频诊断名（CCSR 开放集合常用项，按 2021 年住院量 Top 40 人工翻译；
    # 其余罕见诊断/手术/医院名由 translate_dimensions.py + dim_zh_cache.json 全量补充）。
    # 注意：key 必须与 P3 返回的 description 完全一致（_localize_api_result 为精确值匹配）。
    "LIVEBORN": "活产儿",
    "CORONAVIRUS DISEASE 2019 (COVID-19)": "新型冠状病毒感染(COVID-19)",
    "SEPTICEMIA": "败血症",
    "OSTEOARTHRITIS": "骨关节炎",
    "SCHIZOPHRENIA SPECTRUM AND OTHER PSYCHOTIC DISORDERS": "精神分裂症谱系及其他精神病性障碍",
    "COMPLICATIONS SPECIFIED DURING CHILDBIRTH": "分娩期特定并发症",
    "OBESITY": "肥胖症",
    "DIABETES MELLITUS WITH COMPLICATION": "糖尿病伴并发症",
    "HEART FAILURE": "心力衰竭",
    "SPONDYLOPATHIES/SPONDYLOARTHROPATHY (INCLUDING INFECTIVE)": "脊柱病/脊柱关节病(含感染性)",
    ...
    "HYPERTENSION AND HYPERTENSIVE-RELATED CONDITIONS COMPLICATING PREGNANCY; CHILDBIRTH; AND THE PUERPERIUM": "妊娠、分娩及产褥期高血压及相关并发症",
    "CORONARY ATHEROSCLEROSIS AND OTHER HEART DISEASE": "冠状动脉粥样硬化及其他心脏病",
    "EARLY OR THREATENED LABOR": "早产或先兆临产",
    "PANCREATIC DISORDERS (EXCLUDING DIABETES)": "胰腺疾病(不含糖尿病)",
    "CHRONIC KIDNEY DISEASE": "慢性肾脏病",
    "COMPLICATION OF OTHER SURGICAL OR MEDICAL CARE, INJURY, INITIAL ENCOUNTER": "其他外科或医疗护理并发症、损伤(初始接诊)",
    "MALPOSITION, DISPROPORTION OR OTHER LABOR COMPLICATIONS": "胎位异常、骨盆不称或其他分娩并发症",
    ...
    "MATERNAL CARE RELATED TO FETAL CONDITIONS": "与胎儿情况相关的母体照护",
    "COMPLICATION OF INTERNAL ORTHOPEDIC DEVICE OR IMPLANT, INITIAL ENCOUNTER": "骨科内植物并发症(初始接诊)",
    "GASTROINTESTINAL HEMORRHAGE": "胃肠道出血",
    "INTESTINAL OBSTRUCTION AND ILEUS": "肠梗阻及肠郁张",
    "ASTHMA": "哮喘",
    # —— 高频手术名（CCSR procedure description，2021 Top40，查 DB 实得） ——
    "ADMINISTRATION OF THERAPEUTIC SUBSTANCES, NEC": "治疗药物施用（其他未分类）",
    "SPONTANEOUS VAGINAL DELIVERY": "自然阴道分娩",
    "VACCINATIONS": "疫苗接种",
    "PHARMACOTHERAPY FOR MENTAL HEALTH (EXCLUDING SUBSTANCE USE)": "精神健康药物治疗（不含物质使用）",
    "ISOLATION PROCEDURES": "隔离操作",
    "ADMINISTRATION OF ANTIBIOTICS": "抗生素给药",
    "POTENTIAL COVID-19 THERAPIES": "潜在新冠肺炎治疗",
    "CESAREAN SECTION": "剖宫产",
    "HIP ARTHROPLASTY": "髋关节置换术",
    "DIAGNOSTIC AUDIOLOGY": "诊断听力学检查",
    "CIRCUMCISION": "包皮环切术",
    "KNEE ARTHROPLASTY": "膝关节置换术",
    "NON-INVASIVE VENTILATION": "无创通气",
    "GASTRECTOMY": "胃切除术",
    "ADMINISTRATION OF NUTRITIONAL AND ELECTROLYTIC SUBSTANCES": "营养与电解质物质输注",
    ...
    "HEMODIALYSIS": "血液透析",
    "PHYSICAL, OCCUPATIONAL, AND RESPIRATORY THERAPY TREATMENT": "物理/作业/呼吸治疗",
    "ELECTROENCEPHALOGRAM (EEG)": "脑电图（EEG）",
    "MENTAL HEALTH PROCEDURES, NEC": "精神健康操作（其他未分类）",
    "PERCUTANEOUS CORONARY INTERVENTIONS (PCI)": "经皮冠状动脉介入治疗（PCI）",
    "PSYCHOTHERAPY FOR MENTAL HEALTH (EXCLUDING SUBSTANCE USE)": "精神健康心理治疗（不含物质使用）",
    "MEASUREMENT DURING CARDIAC CATHETERIZATION": "心导管检查测量",
    "PARACENTESIS": "腹腔穿刺术",
    "FEMUR FIXATION": "股骨固定术",
    "CHOLECYSTECTOMY": "胆囊切除术",
    "GASTRO-JEJUNAL BYPASS (INCLUDING BARIATRIC)": "胃空肠旁路术（含减重手术）",
    "FIXATION OF LEG AND FOOT BONES": "腿足骨固定术",
    "SUBSTANCE USE DETOXIFICATION": "物质使用脱毒治疗",
    "GI SYSTEM ENDOSCOPY WITHOUT BIOPSY (DIAGNOSTIC)": "消化道内镜检查（诊断性，无活检）",
    "ARTHROPLASTY OF OTHER JOINT (EXCLUDING KNEE AND HIP)": "其他关节置换术（不含膝、髋）",
...
def _finalize_result(question: str, intent: dict, api_result: dict,
    ...
    # 空结果放宽重试命中时，在答案前提示用户具体条件已被放宽（避免误以为就是原问题口径）
    if (isinstance(api_result, dict)
            and (api_result.get("meta") or {}).get("filters_relaxed")):
        summary = ("⚠️ 未找到您指定的具体条件（病名/医院名等，库里为英文编码），"
                   "已按整体口径展示：\n" + summary)
    ...
    if has_p3_error:
        ...
        # —— 图表 LLM 增强层（混合模式）——
        # 在代码基线 option 之上叠加 LLM 的标题/配色/基准线/图内洞察；
        # 任何字段异常或 LLM 失败都保留基线不变，绝不崩图。
        if chart and isinstance(chart.get("option"), dict) and LLM_ENABLED \
                and os.getenv("LLM_CHART_ENHANCE", "true").lower() in ("1", "true", "yes", "y"):
            try:
                ds = api_result.get("data")
                data_sample = ds[:10] if isinstance(ds, list) else []
                enh = _enhance_chart_by_llm(
                    question, intent.get("chart_hint"), chart.get("chart_type"),
                    chart.get("option"), data_sample)
                if enh:
                    chart["option"] = _apply_chart_enhancement(
                        chart.get("option"), enh, chart.get("chart_type"))
                    chart["_enhanced_by_llm"] = True
            except Exception as e:
                logger.warning("图表 LLM 增强失败，保留代码基线 option：%s", e)
...
def _summary_cost_relation(question, intent, api_result, dim_zh, metric_zh):
    ...
    # 比率最高
    top = _cost_relation_readable(sorted_by_ratio[0])
    bot = _cost_relation_readable(sorted_by_ratio[-1])
    lines.append(f"  🔼 费/本比最高：「{top['label']}」{top['charge_cost_ratio'] or 0} "
                 f"（次均费用 {top['avg_charges']:,.0f} / 次均成本 {top['avg_costs']:,.0f}）")
    lines.append(f"  🔽 费/本比最低：「{bot['label']}」{bot['charge_cost_ratio'] or 0} "
                 f"（次均费用 {bot['avg_charges']:,.0f} / 次均成本 {bot['avg_costs']:,.0f}）")
    # 平均比率
...
def _build_cost_relation_option(intent, api_result, suggestion):
    ...
    option = {
        "title": title_obj,
        "tooltip": {"trigger": "item",
                    "formatter": "{b}<br/>成本: {c0}<br/>费用: {c1}<br/>人数: {c2}<br/>费/本比: {c3}"},
    ...
    for i, r in enumerate(data):
        ...
        costs = float(r.get("avg_costs") or 0)
        charges = float(r.get("avg_charges") or 0)
        ratio = float(r.get("charge_cost_ratio") or 0)
        # label 优先用 P3 已解析的中文 name，否则回退 key（payment/age_group 已是英文/数字）
        label = r.get("name") or r.get("key") or "-"
        series_data2.append({
            "value": [costs, charges, counts[i], ratio],
            "name": label,
            "symbolSize": min(size, 60),
            # ECharts 4 字符串 formatter 不支持 {c0}/{c[0]} 等多维索引，
            # 因此每个数据点预渲染 tooltip，避免前端按维度取值失败。
            "tooltip": {
                "formatter": (
                    f"{label}<br/>成本: {costs:,.2f}<br/>"
                    f"费用: {charges:,.2f}<br/>人数: {counts[i]}<br/>费/本比: {ratio:.2f}"
                )
            },
        })
...
# ------------------------------------------------------------
# 5.9 图表 LLM 增强层（混合模式：代码基线兜底 + LLM 点睛）
# ------------------------------------------------------------
# 设计原则：图表的 series/data 永远由上面的确定性代码生成（准确、零幻觉、永不崩）；
# LLM 只在这一层做"增强"——精炼标题、补副标题、定配色、加基准线标注、写图内一句话洞察。
# 每个增强字段都单独校验，异常即忽略（绝不崩图）；LLM 整体失败则保留代码基线 option 不变。
# 通过环境变量 LLM_CHART_ENHANCE（默认 true）可关闭此层，立即回退到纯代码出图。
_CHART_ENHANCE_SYSTEM_PROMPT = """你是数据可视化设计师，负责给已有的 ECharts 图表做"点睛"增强。
图表的数据 series 已由系统确定性生成（准确、不可改），你只负责提升表达力。

基于用户问题与图表现有信息，输出严格 JSON 的增强指令（不要 markdown、不要任何解释文字）：
{
  "title": "更贴切的图表主标题（≤20字，可选；不填则保留原标题）",
  "subtitle": "副标题/数据口径说明（≤40字，可选）",
  "color_scheme": ["#色1","#色2", ...]（可选，2-8 个合法 hex 颜色，覆盖默认配色）,
  "insight_text": "图内一句话洞察（≤50字，可选，如'活产儿占比突出，远超其他诊断'）",
  "threshold_line": {"value": 数值, "label": "基准线说明（≤15字，可选）"}
}

约束：
1. 忠于图表真实数据，禁止编造数字或新维度。
2. threshold_line 仅用于有数值轴的柱状/折线图，value 应是有业务含义的基准（如均值、目标值），不要乱设。
3. 没有合适增强就返回空对象 {}。
4. 数据中英文维度取值（F/M、Medicare、Minor、疾病名等）一律用中文表达。
"""


def _is_hex_color(s) -> bool:
    return isinstance(s, str) and bool(re.fullmatch(r"#[0-9a-fA-F]{3}|#[0-9a-fA-F]{6}", s))


def _enhance_chart_by_llm(question: str, chart_hint: str, chart_type: str,
                          baseline_option: dict, data_sample: list) -> dict:
    """让 LLM 产出图表的增强指令（JSON），返回解析后的 dict（可能为空 {}）。

    任何异常都返回 {}，调用方据此保留代码基线 option。
    """
    if not (LLM_ENABLED and os.getenv("LLM_CHART_ENHANCE", "true").lower() in ("1", "true", "yes", "y")):
        return {}
    if not isinstance(baseline_option, dict):
        return {}

    # 组装给 LLM 的上下文（只给关键信息，省 token）
    ctx_lines = [f"用户问题：{question}", f"图表类型：{chart_type}"]
    if chart_hint:
        ctx_lines.append(f"分析维度：{CHART_HINT_TITLE_ZH.get(chart_hint, chart_hint)}")
    if isinstance(data_sample, list) and data_sample:
        top = []
        for it in data_sample[:5]:
            if isinstance(it, dict):
                k = it.get("key") or it.get("year") or it.get("payment") or it.get("name")
                v = it.get("value") if it.get("value") is not None else it.get("count")
                if k is not None and v is not None:
                    top.append(f"{k}={v}")
        if top:
            ctx_lines.append("Top 数据：" + "，".join(top))
    ctx = "\n".join(ctx_lines)

    messages = [
        {"role": "system", "content": _CHART_ENHANCE_SYSTEM_PROMPT},
        {"role": "user", "content": f"【图表信息】\n{ctx}\n\n请输出增强指令 JSON："},
    ]
    ok, content = _call_llm_safely(messages)
    if not ok:
        return {}
    ok_json, data, _ = _extract_json_from_llm_output(content)
    if not ok_json or not isinstance(data, dict):
        return {}
    return data


def _apply_chart_enhancement(option: dict, enh: dict, chart_type: str) -> dict:
    """把 LLM 增强指令安全地套到基线 option 上；任何字段异常都忽略，绝不崩图。"""
    if not isinstance(enh, dict) or not enh or not isinstance(option, dict):
        return option
    option = copy.deepcopy(option)  # 深拷贝，保证纯函数、绝不污染调用方基线

    # 标题
    title = enh.get("title")
    if isinstance(title, str) and 0 < len(title.strip()) <= 30:
        t = dict(option.get("title") or {})
        t["text"] = title.strip()
        option["title"] = t
    # 副标题
    subtitle = enh.get("subtitle")
    if isinstance(subtitle, str) and 0 < len(subtitle.strip()) <= 50:
        t = dict(option.get("title") or {})
        t["subtext"] = subtitle.strip()
        t.setdefault("itemGap", 8)
        option["title"] = t
    # 配色
    cs = enh.get("color_scheme")
    if isinstance(cs, list):
        hexes = [c for c in cs if _is_hex_color(c)]
        if 2 <= len(hexes) <= 8:
            option["color"] = hexes
    # 图内洞察（下划线键，前端可选显示在图下方，不影响渲染）
    insight = enh.get("insight_text")
    if isinstance(insight, str) and 0 < len(insight.strip()) <= 60:
        option["_insight"] = insight.strip()
    # 基准线标注（仅数值轴图表：bar/line）
    tl = enh.get("threshold_line")
    if isinstance(tl, dict) and chart_type in ("bar", "line"):
        val = tl.get("value")
        label = tl.get("label")
        try:
            val = float(val)
        ...
        if val is not None:
            ...
            if isinstance(series, list) and series and isinstance(series[0], dict):
                s0 = series[0]
                mk = s0.get("markLine") or {
                    "silent": True, "symbol": "none",
                    "lineStyle": {"type": "dashed", "color": "#e74c3c"},
                    "label": {"formatter": "{b}"}, "data": [],
                }
                mk = dict(mk)
                mk.setdefault("data", [])
                entry = {"yAxis": val}
...
# ------------------------------------------------------------
_REPORT_SYSTEM_PROMPT = """你是智慧医疗大数据平台的「资深医学数据分析师」，最擅长把枯燥的统计数字讲成有业务价值的洞察。

任务：基于下面按维度分段提供的结构化分析结果，输出一份 JSON 格式洞察报告。

【写作铁律——请真正发挥分析能力，严禁套模板（违反直接判不合格）】
1. 忠于数据，禁止编造数字；所有结论都要能从输入数据推出。
2. 严禁以下句式（一旦发现即不合格）：
   ❌ "X 是 Y，表明/意味着/反映了……"（逐条数字 + 泛泛解读）
   ❌ "针对 X 的情况，医院应……" / "鉴于 X，医院应……"（建议千篇一律的开头）
   ❌ "排名第 N 的是 X，指标值为 Y，是第二名的 M 倍"（机械罗列）
3. 每个维度写 2-4 条有层次、角度交替的洞察：可对比（"接近第二名的 2 倍"）、可算占比
   （"占住院 12%"）、可点业务含义（"新生儿护理需前置"）、可提反常（"成本占比异常低"）。
   不要每条都是"数字 + 解读"的同构句式，角度要变。
4. 建议（recommendations）要具体到"对谁、做什么、基于哪个数字"。例如：
   ✅ "对占比 40% 的联邦医保患者，提前接入预授权与病种打包支付，压缩平均住院中的等待环节"
   ❌ "针对联邦医保占比高，医院应优化报销流程"（空话，禁止）
   3-4 条，开头句式务必多样化（可用疑问句引出、可用"优先""值得关注""建议"等不同动词）。
5. 英文维度取值一律译中文（Medicare→联邦医疗保险、Minor→轻度、活产儿等）。
6. 输出严格合法 JSON，无 markdown、无解释文字。

【维度分段处理】
- KPI / 大屏类（输入给的是完整指标卡片 JSON）：不要逐条复述每个数字，而是写一段
  连贯叙事，把最关键的 2-3 个信号（如"规模 / 费用结构 / 效率"三者关系）串起来，
  并点出最值得管理层关注的矛盾点（如"费用高但住院短，控费重点在结构而非时长"）。
- 排行榜 / 分布类（输入是 TopN 列表）：抓最突出者，讲清"为什么值得关注、意味着什么"。

输入已按【维度N】分段，sections 数组顺序与输入一一对应，dimension 字段照抄输入维度名。

输出格式：
{
  "summary": "一句话讲清分析了什么数据、覆盖多少条、最值得关注的核心结论（50-100字，避免罗列数字）",
  "sections": [
    {"dimension": "维度名（照抄输入）", "key_findings": ["洞察1", "洞察2", "洞察3"]},
    {"dimension": "维度名（照抄输入）", "key_findings": ["洞察1", "洞察2"]}
  ],
  "recommendations": ["具体到数字的可执行建议1", "建议2", "建议3"]
}

【关系/散点类专项要求】
- 若输入是「费用-成本关系」等成对数值（每个分组含 次均费用、次均成本、费用成本比）：
  不要逐条罗列"X指标值Y"或机械排名；要讲：① 费用与成本整体是否近共线（比值是否集中）；
  ② 哪些分组费本比显著偏高/偏低（意味着定价或成本结构异常）；③ 异常点意味着什么业务含义。
- 严禁把分组 key 当作"记录编号"编造"记录N"之类表述；直接引用数据中的分组名(label)与真实数字。
"""
...
def _generate_report_by_llm(context: str) -> tuple[bool, dict, str]:
    ...
    # 按维度的灵活洞察（用于替换死板的 rule_findings）
    ...
    return True, {
        "summary": summary,
        "sections": llm_sections,   # 顺序与输入维度一一对应
...
# —— cost_relation（费用-成本关系散点）的分组可读化映射 ——
# 字段 key 在 P3 侧含义：by=payment → payment_typology_1 英文（Medicare 等，已 DB 实查）；
# by=age_group → "0 to 17" 等英文；by=diagnosis → diagnosis_id 数字码（P3 另返回 name 中文名）。
PAYMENT_TYPOLOGY_ZH = {
    "Medicare": "联邦医疗保险(Medicare)",
    "Medicaid": "医疗补助(Medicaid)",
    "Private Health Insurance": "商业健康保险",
    "Blue Cross/Blue Shield": "蓝十字/蓝盾保险",
    "Self-Pay": "自费",
    "Miscellaneous/Other": "其他",
    "Managed Care, Unspecified": "管理式医疗(未特指)",
    "Federal/State/Local/VA": "政府/退伍军人事务",
...
def _cost_relation_readable(item: dict) -> dict:
    """把 cost_relation 的单个数据项转成『可读标签 + 结构化数值』。

    用于报告生成（喂 LLM 真实数据）与保底 findings：
      - label：优先 P3 已给的 name（diagnosis 中文诊断名）；否则 payment/age_group
        走中文映射；再否则用原始 key（避免数字码被 LLM 编成「记录N」）。
      - 其余字段保持原语义：count 记录数、avg_charges 次均费用、
        avg_costs 次均成本、charge_cost_ratio 费/本比（费用÷成本）。
    """
    if not isinstance(item, dict):
        return {"label": "-", "count": 0, "avg_charges": 0, "avg_costs": 0, "charge_cost_ratio": None}
    raw_key = item.get("key") or "-"
    name = item.get("name")
    if name:
        label = str(name)
    elif raw_key in PAYMENT_TYPOLOGY_ZH:
        label = PAYMENT_TYPOLOGY_ZH[raw_key]
    elif raw_key in AGE_GROUP_ZH:
        label = AGE_GROUP_ZH[raw_key]
    else:
        label = str(raw_key)
    return {
        "label": label,
        "count": item.get("count", 0) or 0,
        "avg_charges": item.get("avg_charges", 0) or 0,
        "avg_costs": item.get("avg_costs", 0) or 0,
        "charge_cost_ratio": item.get("charge_cost_ratio"),
    }


def _build_cost_relation_findings(data) -> list[str]:
    """cost_relation（散点关系）的保底洞察：聚焦费/本比高低与均值，而非机械排名。

    LLM 生成失败时兜底用；返回 2-3 条。
    """
    if not isinstance(data, list) or not data:
        return []
    items = [_cost_relation_readable(it) for it in data]
    valid = [it for it in items if isinstance(it.get("charge_cost_ratio"), (int, float))]
    if not valid:
        return []
    valid.sort(key=lambda x: x["charge_cost_ratio"], reverse=True)
    ratios = [it["charge_cost_ratio"] for it in valid]
    avg_ratio = sum(ratios) / len(ratios)
    findings = []
    top = valid[0]
    bot = valid[-1]
    findings.append(
        f"费/本比最高的是「{top['label']}」，达 {top['charge_cost_ratio']:.2f}"
        f"（次均费用 {top['avg_charges']:,.0f} / 次均成本 {top['avg_costs']:,.0f}），"
        f"提示其定价或成本结构明显偏离常规"
    )
    if bot is not top:
        findings.append(
            f"费/本比最低的是「{bot['label']}」，仅 {bot['charge_cost_ratio']:.2f}"
            f"（次均费用 {bot['avg_charges']:,.0f} / 次均成本 {bot['avg_costs']:,.0f}）"
        )
    findings.append(
        f"整体平均费/本比 {avg_ratio:.2f}；高于 3.0 的分组（共 "
        f"{sum(1 for r in ratios if r > 3.0)} 组）需关注定价合理性"
    )
    return findings
...
def _build_section_findings(chart_hint: str | None, data, meta: dict) -> list[str]:
    ...
    # —— 特殊 0：cost_relation 的 data 是 list（散点关系），按费/本比高低写保底洞察 ——
    if chart_hint == "cost_relation":
        return _build_cost_relation_findings(data)
...
def generate_insight_report(single_result: dict = None, multi_results: list = None,
    ...
    for i, r in enumerate(results):
        ...
        is_relation = (chart_hint == "cost_relation")
        ...
        # cost_relation：把原始数据点转成可读 + 结构化，供 LLM 基于真实数据写洞察
        # （替代 rule 的「排名第N指标值」模板，避免 LLM 把数字 key 编成「记录N」）
        relation_raw = (
            [_cost_relation_readable(it) for it in data[:12]]
            if is_relation and isinstance(data, list) else None
        )

        report["sections"].append({
            # 单维度报告不显示"分析维度 N"编号，直接以维度名作标题（前端渲染无感，字段结构不变）
            "section_title": section_dim_zh,
        ...
        sections_meta.append({
            ...
            "raw_data": relation_raw if is_relation else (data if is_dict_data else None),
        })
    ...
    if use_llm and LLM_ENABLED and sections_meta:
        ...
        for i, sm in enumerate(sections_meta, 1):
            ...
            if sm.get("is_kpi") and sm.get("raw_data") is not None:
                # KPI / 大屏：直接给原始指标 JSON，让 LLM 从零综合成连贯叙事，
                # 而不是照着代码预生成的"逐条发现"改写成"X是Y表明Z"模板
                head += (
                    f"   原始KPI数据(JSON): {json.dumps(sm['raw_data'], ensure_ascii=False)}\n"
                    f"   （请基于原始数据写一段连贯、有层次的洞察，把最关键的 2-3 个信号串起来；"
                    f"不要逐条复述数字，严禁'X是Y表明Z'句式）"
                )
            elif sm.get("is_relation") and sm.get("raw_data"):
                # 关系/散点类：直接给真实数据点（已可读化），让 LLM 基于真实数值讲分布/异常，
                # 而不是照着"排名第N指标值"模板编造
                head += (
                    f"   这是「费用-成本关系」散点数据：每个分组含 次均成本(avg_costs)、"
                    f"次均费用(avg_charges)、记录数(count)、费用成本比(charge_cost_ratio=费用÷成本)。\n"
                    f"   原始数据点(JSON, 共 {len(sm['raw_data'])} 组): "
                    f"{json.dumps(sm['raw_data'], ensure_ascii=False)}\n"
                    f"   （请基于真实数据写 2-3 条洞察：聚焦①费用与成本整体是否近共线（比值是否集中）；"
                    f"②哪些分组费本比显著偏高/偏低、意味着定价或成本结构异常；③异常点的业务含义。"
                    f"严禁把分组 label 当成'记录编号'编造'记录N'，严禁'指标值X表明Y'句式，"
                    f"直接引用数据中的分组名(label)与真实数字）"
                )
            else:
                head += f"   Top3发现: {'; '.join(sm['top3'])}"
        ...
        if ok:
            ...
            # 用 LLM 按维度灵活写的 key_findings 替换死板的 rule_findings
            # （rule_findings 已在每节预置，仅当 LLM 该维度无产出时保留）
            llm_sections = llm_report.get("sections") or []
            # 维度名 -> section 下标（用于按名匹配，容忍 LLM 顺序偏差）
            dim_to_idx = {
                sec.get("meta", {}).get("dimension_zh"): i
                for i, sec in enumerate(report["sections"])
            }
            for order, lf in enumerate(llm_sections):
                if not isinstance(lf, dict):
                    continue
                findings = lf.get("key_findings") or []
                if not isinstance(findings, list) or not findings:
                    continue
                # 优先按维度名匹配输入，否则按输入顺序兜底
                idx = dim_to_idx.get(lf.get("dimension", ""))
                if idx is None and order < len(report["sections"]):
                    idx = order
                if idx is not None and idx < len(report["sections"]):
                    report["sections"][idx]["key_findings"] = [
                        str(x).strip() for x in findings[:4] if str(x).strip()
                    ]
...
def _web_search(query: str, top_n: int = WEB_SEARCH_TOP_N) -> list[dict]:
    """用 Bing（国内可访问，无需 key）抓取搜索结果，返回 [{title, url, snippet}]。

    抓取失败/无结果返回空列表（调用方自动降级为纯 LLM 回答）。依赖 Bing 的 HTML
    结构（.b_algo > h2>a 标题 + .b_lineclamp 摘要），结构变化时静默返回空。
    """
    import html as _html
    if not query:
        return []
    # 去掉疑问前缀/语气词，提高搜索引擎命中率（"什么是DRG付费" → "DRG付费"）
    query = re.sub(r"^(什么是|什么叫|请问一下|请问|啥是|如何|怎样|怎么|为什么|为何|谁|哪里|哪些|哪个)\s*",
                   "", query).strip()
    if not query:
        return []
    try:
        ...
        logger.warning("[web_search] Bing 搜索失败：%s", e)
        return []
    ...
    for b in blocks[:top_n]:
        ...
        if not m:
            continue
        title = _html.unescape(re.sub("<[^>]+>", "", m.group(2))).strip()
        url = _html.unescape(m.group(1)).strip()
        sn = re.search(r'<p[^>]*class="[^"]*b_lineclamp[^"]*"[^>]*>(.*?)</p>', b, re.S)
        snippet = _html.unescape(re.sub("<[^>]+>", "", sn.group(1))).strip() if sn else ""
        if title:
...
def general_chat():
    """通用问答接口（前端「通用」模式用）：先联网搜索（Bing）→ 把结果喂给 LLM 回答。

    请求 JSON: { "question": "...", "history": [{"role":"user/assistant","content":"..."}] }
    返回 JSON: { "code": 0, "message": "success", "answer": "...", "sources": [...] }  或
              { "code": 1, "message": "错误原因", "answer": null }
    """
    body = request.get_json(silent=True) or {}
    question = (body.get("question") or "").strip()
    history = body.get("history") or []
    if not question:
        return jsonify({"code": 1, "message": "问题不能为空", "answer": None}), 200

    # 1) 联网搜索
    sources = _web_search(question) if WEB_SEARCH_ENABLED else []
    search_ctx = ""
    if sources:
        search_ctx = "\n\n以下是从网络搜索到的最新资料（仅作参考）：\n"
        for i, r in enumerate(sources, 1):
            search_ctx += f"{i}. {r['title']}\n   {r['url']}\n   {r['snippet']}\n"

    # 2) 组装消息
    messages = [{
        "role": "system",
        "content": "你是智慧医疗大数据分析助手。回答问题时优先参考用户消息里附带的最新"
                   "网络搜索结果，结合资料给出准确、及时的回答；若资料与问题无关或为空，"
                   "再基于自身知识回答。中文回答，简洁专业；不确定时如实说明。",
    }]
    for h in history[-6:]:
        if isinstance(h, dict) and h.get("role") and h.get("content"):
            messages.append({"role": h["role"], "content": h["content"]})
    user_content = question + search_ctx
    messages.append({"role": "user", "content": user_content})

    # 3) 调 LLM
    ok, text = _call_llm_safely(messages, use_small_model=False)
    if ok:
        return jsonify({"code": 0, "message": "success", "answer": text,
                        "sources": sources}), 200
    return jsonify({"code": 1, "message": f"LLM 调用失败：{text}", "answer": None}), 200
...
def suggested_questions():
    """P5 前端『推荐问题』数据源：覆盖全部 24 个分析工具，单一来源。
